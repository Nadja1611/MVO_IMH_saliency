...
from linear_probe_utils import (
    features_dataloader,
    train_multilabel_timing,
    train_multilabel,
    LinearClassifier,
    Finetune_Classifier_timing
)
...
def main(config):
    ...
    for raw_id in ID_train:
        ...
        binary = torch.tensor([1, 0]) if timing_value > 0 else torch.tensor([0, 1])
        binary_labels.append(binary)

    # Stack all tensors into one tensor [N, 2]
    binary_labels_tensor = torch.stack(binary_labels)
    print('the shape is', binary_labels_tensor.shape, flush=True)
    print(binary_labels_tensor, flush=True)
    # Concatenate to original multilabel labels [N, 3] → [N, 4]
# shape: [N, 4]
    ...
    for i, (train_files, val_files, test_files) in enumerate(bootstrap_splits):
      ...
      #  waves_train = np.concatenate((waves_train,waves_val))
        ...
        print(f"binary_labels_tensor shape: {binary_labels_tensor.shape}", flush = True)
        print(f"train_idx: {train_idx}", flush = True)
        print(f"val_idx: {val_idx}", flush = True)
        labels_val = all_labels[val_idx]
        labels_val_time = binary_labels_tensor[val_idx]
        labels_train_time = binary_labels_tensor[train_idx]

       # labels_train_time = np.concatenate((binary_labels_tensor[train_idx], binary_labels_tensor[val_idx]))
        
        labels_train = all_labels[train_idx]
       # labels_train = np.concatenate((labels_train, labels_val))
        labels_test = all_labels[test_idx]
        labels_test_time = binary_labels_tensor[test_idx]
        labels_test = np.concatenate((labels_test, np.array(labels_test_time)), 1)
        labels_val_time = binary_labels_tensor[val_idx]
        labels_val = np.concatenate((labels_val, np.array(labels_val_time)), 1)
        labels_train = np.concatenate((labels_train, np.array(labels_train_time)), 1)
        print(labels_train.shape, flush = True)
        print(labels_train[:6], flush = True)
        ...
        linear_model = LinearClassifier(embed_dim, 3).to(device)
        linear_model_timing = LinearClassifier(embed_dim, 2).to(device)

        optimizer = optim.AdamW(linear_model.parameters(), lr=lr)
        optimizer_timing = optim.AdamW(linear_model_timing.parameters(), lr=lr)
        ...
        scheduler_timing = CosineLRScheduler(
            optimizer_timing,
            t_initial=num_epochs * iterations_per_epoch,
            cycle_mul=1,
            lr_min=lr * 0.1,
        ...
        train_dataset = ECGDataset(waves_train_selected, labels_train_selected, train_transforms)
        val_dataset = ECGDataset(waves_val_selected, labels_val_selected, test_transforms)
        ...
        full_model = Finetune_Classifier_timing(encoder, linear_model, linear_model_timing).to(device)
        # LR = 0.007
        LR = 0.007
        #LR = 0.01
        LR_FT = lr * LR
    ...
    # writer = SummaryWriter(log_dir=str(log_dir))
        ...
        optimizer = optim.AdamW([
            ...
            {'params': linear_model_timing.parameters(), 'lr': LR_FT*2},
        ...
        if config["task"] == "multilabel":
            acc, acc_per_class, auc, auc_per_class, f1, f1_per_class, auprc_macro, auprc_per_class, recall_macro, recall_per_class,precision_macro, precision_per_class, y_probs, y_true = train_multilabel_timing(
                num_epochs,
                full_model,
                optimizer,
                criterion,
        ...
        for i in range(n_classes):
            ...
            ## save 
            filename = f"/home/nadja/MVO_Project_Paper/ROC_results/auc_all_all/Paper_FT_fold_{i}_metrics.npz"
            np.savez(filename,
             mean_tpr=mean_tpr,
             std_tpr=std_tpr,
             mean_auc=mean_auc,
