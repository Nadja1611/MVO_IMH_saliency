...
class MaskTransformer(nn.Module):
    def __init__(
        ...
        self.encoder_blocks = Encoder_Block(
            embed_dim=embed_dim,
            depth=depth,
            num_heads=num_heads,
            mlp_ratio=mlp_ratio,
    ...
    def _make_rand_mask(self, mask_scale):
        ...
        mask_ratio = (
            mask_scale[0] + (mask_scale[1] - mask_scale[0]) * torch.rand(1).item()
        )
    ...
    def forward(self, x, mask=None):
        ...
        # Apply projection to input tensor (nn.Linear(t,embed_dim))
        ...
        if mask is not None:
            ...
            pos_embed = pos_embed[
                :, vis_idx
            ]  # (bs, c, p, embed_dim) -> (bs, c, p', embed_dim)
    ...
    def restrict_leads(self, vec, type="vector"):
        assert vec.size(0) == self.p * self.c, (
            f"Input.size(0) should be c*p, vec.size(0)={vec.size(0)}"
        )
        assert vec.dim() == 2, f"Input should be of dimension 2, vec.dim()={vec.dim()}"
        assert type in ["vector", "matrix"], (
            f"type should be either vector or matrix, type={type}"
        )
        ...
        if type == "matrix":
            assert vec.size(1) == self.c * self.p, (
                f"Input.size(1) should be c*p, vec.size(1)={vec.size(1)}"
            )
    ...
    def representation(self, x):
        ...
        assert x.shape[2] == 1250, (
            f"Input should be of shape (bs, c, 2500), x.shape[2]={x.shape[2]}"
        )
...
class MaskTransformerPredictor(nn.Module):
    def __init__(
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]
        self.predictor_blocks = Predictor_Block(
