...
def get_mean_signals(waves_test, labels_test, label = 'IMH'):
    n = 0
    all_waves = []
    all_contribs = []
...
if interest == 'IMH+MVO':
    ...
    if sal == 'gt':
        mask = (labels_test[:, 0] == 1) & (labels_test[:, 1] == 0)
    else:
        mask = (preds[:, 0] == 1) & (preds[:, 1] == 0)

# Apply mask to only MVO positive patients
waves_test_masked = waves_test[mask]
...
if sal == 'gt':
    print('we plot by gt')
    labels_masked = labels_test[mask]
    mean_hb, mean_sal= get_mean_signals(waves_test_masked, labels_masked, label = 'IMH')
if sal == 'pred':
    print('we plot by prediction')
    preds_masked = preds[mask]
    mean_hb, mean_sal= get_mean_signals(waves_test_masked, preds_masked, label = 'IMH')
#%%
plot_sal = mean_sal.mean(axis=0)/np.max(np.abs(mean_sal.mean(axis=0)))

#%%
custom_cmap = LinearSegmentedColormap.from_list("petrol_white_darkred", colors)
fig, axarr = plot_attributions.plot_attribution(np.mean(mean_hb, axis=0).T, plot_sal.T, cmap=custom_cmap, vscale=(-1,1),ncols=4,
...
if interest == 'IMH+MVO':
    plt.suptitle(f'Average saliency of MVO$^+$/IMH$^+$ ECGs', fontsize=24)
else: 
    plt.suptitle(f'Average saliency of MVO$^+$/IMH$^-$ ECGs', fontsize=24)    
#plt.savefig(f'/Users/nadjagruber/Documents/MVO_Project_paper/plots/MVO_IMH_mean_saliency.svg')



# %%
import pandas as pd
file_path = '/Users/nadjagruber/Documents/MVO_Project_Paper/excels/overview_excel_with_times.xlsx'
df = pd.read_excel(file_path)
laufnummer_tabelle = [p.split('_')[0] for p in pat_masked]

localisation = np.array([
    df.loc[df['Laufnummer'].astype(str) == x, 'Infarktlokalisation'].values[0] 
    for x in laufnummer_tabelle
])
...
if loc == 'anterior':
        ...
        mask = localisation == 0
waves_test_masked = waves_test_masked[mask]
pat_masked = pat_masked[mask]
...
#%%
custom_cmap = LinearSegmentedColormap.from_list("petrol_white_darkred", colors)
fig, axarr = plot_attributions.plot_attribution(np.mean(mean_hb, axis=0).T, mean_sal.mean(axis=0).T, cmap=custom_cmap, vscale=(-0.01,0.01),ncols=4,
                                                gridspec_kw={"hspace": 0.09, "wspace": 0.09, "top": 0.92}, sharey=True)
for ax, name in zip(axarr.T.ravel(), channel_names):
    plot_attributions.add_channel_label(ax, name)
    ax.set_xticks([])
    ...
    for spine in ax.spines.values():
        spine.set_linewidth(2.5)   # change 2 to a larger value if you want even thicker lines
if interest == 'IMH+MVO' and loc == 'anterior':
    plt.suptitle(f'Average saliency of anterior STEMI and  MVO$^+$/IMH$^+$ ECGs', fontsize=24)
elif interest == 'IMH-MVO' and loc == 'anterior': 
    plt.suptitle(f'Average saliency  of anterior STEMI and MVO$^+$/IMH$^-$ ECGs', fontsize=24)  
elif interest == 'IMH+MVO' and loc == 'non-anterior': 
    plt.suptitle(f'Average saliency  of non-anterior STEMI and MVO$^+$/IMH$^+$ ECGs', fontsize=24)  
else: 
    plt.suptitle(f'Average saliency of non-anterior MVO$^+$/IMH$^-$ ECGs', fontsize=24)    
#plt.savefig(f'/Users/nadjagruber/Documents/MVO_Project_paper/plots/MVO_IMH_mean_saliency.svg')
