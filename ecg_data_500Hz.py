...
def waves_ibk(data_dir, reduced_lead=True):
    ...
    ecg_data = get_ecg_data(data_dir, reduced_lead=reduced_lead)
    """ lead shift """
...
def waves_ptbxl_training(
    data_dir, task="multilabel", reduced_lead=True, downsample=False
):
    ...
    if downsample:
        ...
        ecg_data = np.array(
            [resample(ecg_data[i], 5000, axis=1) for i in range(len(data))]
        )
...
def waves_ptbxl(data_dir, task='multilabel', reduced_lead=True, downsample=False):
    ...
    np.savez_compressed('/home/nadja/MVO_Project_Paper/downstream/ptbxl.npz', data[:10])

    ''' lead shift '''
    ecg_new = np.zeros((data.shape[0], 8, 2500))
    ecg_new[:,:2] = data[:,:2,:2500]
    ecg_new[:,2:] = data[:,6:,2500:]
    data = ecg_new
