...
plt.savefig(f'MVO 12-36 {fold} target {target}.svg')
...
#%%
hb, sal= get_mean_signals(waves_test[idx:idx+1], preds[idx:idx+1], label = 'MVO')
fig, axarr = plot_attributions.plot_attribution(np.mean(hb, axis=0).T, sal.mean(axis=0).T, cmap=custom_cmap, vscale=(-0.015, 0.015),ncols=4,
                                                gridspec_kw={"hspace": 0.09, "wspace": 0.09, "top": 0.92}, sharey=True)
for ax, name in zip(axarr.T.ravel(), channel_names):
    plot_attributions.add_channel_label(ax, name)
    ax.set_xticks([])
    ...
    for spine in ax.spines.values():
        spine.set_linewidth(2)   # change 2 to a larger value if you want even thicker lines
    plt.suptitle(f'MVO$^+$/IMH$^+$ non-anterior infarction', fontsize=24)
    plt.savefig(f'/Users/nadjagruber/Documents/MVO_Project_paper/plots/MVO_IMH_example_non-anterior_saliency.svg')
