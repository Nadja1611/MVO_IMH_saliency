...
def main(config):
    ...
    print('shape of ecgs, ', waves_train.shape, flush = True) 
    ...
    splits = crossval_split(patients, task='multilabel')
    # === Define transforms ===
    aug = {
        "rand_augment": {
            "use": True,
    ...
    for i, (train_files, val_files, test_files) in enumerate(splits):
        print('Files in testing:', test_files, flush=True)
        test_ids  = set(splits[i][2])
        ...
        if train_val_overlap:
            print(f"[Iteration {i}] Train-Val Overlap (normal if not explicitly split): {train_val_overlap}")
        ...
        linear_save_path = weight_dir / f"weights_fold_{i}.pth"
        ...
        if config["task"] == "multilabel":
           ...
           # all_labels = []
            ...
            #with torch.no_grad():
             #   for batch in test_loader:
               ...
               #     embeddings = encoder(inputs)
                    ...
                    # If needed (e.g., embeddings are not flat), flatten them
            ...
            # Save as .npy files
           # np.save(embedding_dir / f"embeddings_test_fold_{i}.npy", all_embeddings)
           # np.save(embedding_dir / f"labels_test_fold_{i}.npy", all_labels)

          #  print(f"Saved test embeddings to {embedding_dir}")
        ...
        #if config['validation'] == 'crossval':
        #    pred_dir = '../predictions_FT/'
         #   pred_dir.mkdir(parents=True, exist_ok=True)

            #np.savez_compressed(...., probs = all_y_probs, targets = all_y_true)
            #model_save_path = weight_dir / f"weights_{i}.pth"
